...
## 34. v4.2 Extension: Document Type Filtering & Expanded Retrieval

### 34.1 Document ORM Model Extension
Add `document_type` field to `Document` ORM model (table `documents`):
```python
document_type: str  # 'datasheet' | 'manual' | 'brochure' | 'installation_guide' | 'technical_doc' | 'warranty_doc'
```
Qdrant `documents_v1` payload per point is extended to include `document_type: str`.

**Migration:**
```sql
ALTER TABLE documents ADD COLUMN document_type TEXT NOT NULL DEFAULT 'technical_doc';
```
Qdrant collection must be re-ingested after migration to populate the `document_type` payload field.

### 34.2 ExtractedFilters Extension
```python
class ExtractedFilters(BaseModel):
    brand: str | None = None
    category: str | None = None
    spec_filters: dict[str, str] = {}
    doc_type: str | None = None       # NEW: maps intent to document type
    use_case: str | None = None       # NEW: for use-case BOM retrieval
```

**Intent → `doc_type` Mapping** (resolved in `FilterExtractor.extract` or `RetrievalService`):
| Intent | `doc_type` |
|---|---|
| `installation_guidance` | `installation_guide` |
| `warranty_information` | `warranty_doc` |
| `pdf_documentation_search` | `None` (unfiltered; matches all types) |
| `troubleshooting` | `manual` |
| `technical_support` | `technical_doc` |
| All others | `None` (no type filter) |

### 34.3 New Service Method: `retrieve_docs_by_type`
```python
...
```
Extends `retrieve_docs` with an optional `doc_type` Qdrant payload filter. If `doc_type` is provided, the Qdrant query adds `{"key": "document_type", "match": {"value": doc_type}}` to the filter clause. Called by the `retrieve_docs` tool when `intent_result.intent` maps to a specific doc type.

### 34.4 New Repository Method
`DocumentRepository.find_by_type(tenant_id, doc_type: str) -> list[UUID]` — SQL query on `documents.document_type`. Used for type-scoped SQL narrowing before Qdrant search.

### 34.5 Ingestion Script Extension
`scripts/ingest_products_and_docs.py` must accept `--doc-type` CLI argument when ingesting documents:
```bash
python scripts/ingest_products_and_docs.py --type docs --doc-type manual --source ./manuals/
```
Default `doc_type` if omitted: `technical_doc`.
