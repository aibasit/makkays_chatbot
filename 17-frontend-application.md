...
| `types/chat.ts` | `ChatMessage`, `ChatResponse` TypeScript types generated automatically from the backend schemas via the typegen script |

## 8. Classes
Not applicable in the OOP sense (React functional components + hooks only, per the stated stack — no class components).

## 9. Data Models
Client-side types in `types/chat.ts` are generated automatically by running the backend typegen script: **`python scripts/generate_typescript_types.py`** (described in Module 15 §19). This prevents API contract drift.
```ts
...
## 10. Pydantic Schemas
N/A (frontend uses TypeScript types generated automatically from the backend's Pydantic schemas via the typegen script to avoid manual coding drift).
...
- `useChat(): { messages: ChatMessage[], sendMessage(text: string): void, isLoading: boolean, isRateLimited: boolean, rateLimitCooldownSeconds: number, error: string | null, retryLastMessage(): void }` — the single interface `ChatPage` consumes.
  ...
  - `sendMessage(text)`: validates non-empty/non-whitespace, appends a user `ChatMessage` optimistically, calls `postChatMessage`, on success appends the assistant reply; on error keeps the optimistically-appended user message visible and sets `error` so the retry button can resend the same text.
...
## 19. Business Logic
- **Optimistic UI**: user's message is appended to `messages` immediately on send via `useChat.sendMessage`; the assistant's reply is appended only once the response arrives. On error, the optimistically-appended user message remains visible, and `error` is set to display an error bubble with a retry button.
...
- `test_useChat_keeps_optimistic_message_on_error`
...
## 33. Hardening Update: API Contract Alignment
Frontend behavior mirrors Module 15 exactly: request body is `{ message }`, response body is `{ assistant_message, intent, awaiting_clarification }`, and `withCredentials: true` is required for the session cookie. Error handling follows Module 00 §14 and Module 15: 401 shows configuration error, 429 shows cooldown, and 5xx/network errors keep the user message visible with retry.
