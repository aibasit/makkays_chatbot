...
Makkays only sells networking and IT hardware: switches, routers, wireless access
...
Makkays doesn't sell (a laptop, a phone, a car, a personal/relationship question,
...
are reserved for guidance/solutions built from Makkays' own catalog.

## Supported intents

- `sales_inquiry` — product discovery, recommendations, specs, availability, or general buying help, about Makkays' own product categories.
...
- `out_of_scope` — unrelated to Makkays' products/services, including off-topic chat, personal advice, or requests about products Makkays doesn't sell.
- `product_comparison` — comparing two or more Makkays products.
- `product_compatibility` — whether one Makkays product works with another.
- `accessory_recommendation` — add-ons or accessories for a Makkays product.
- `product_finder_by_problem` — describing a problem and needing a Makkays product that solves it.
- `product_alternative` — a replacement or substitute for a Makkays product.
- `specification_explainer` — explaining a technical term or spec (e.g. "what is PoE").
- `product_recommendation_wizard` — open-ended "help me choose" guidance, specifically about Makkays' own product categories (e.g. UPS sizing, network gear selection) — not general shopping advice for unrelated products.
- `use_case_recommendation` — a Makkays product setup for a specific environment (school, hospital, data center, etc.).
- `installation_guidance` — how to install or set up a Makkays product.
...
- `solution_builder` — a full multi-product Makkays setup or bill of materials (e.g. a whole data center or network buildout) — not a single generic product recommendation.
